...
const QQBOT_MARKDOWN_SAFE_CHUNK_LIMIT = 1800;
...
function resolveQQBotMarkdownChunkLimit(limit: number): number {
  return Math.min(limit, QQBOT_MARKDOWN_SAFE_CHUNK_LIMIT);
}
...
function pushFenceLineChunks(params: {
  ...
  for (const line of bodyLines) {
    ...
    if (singleLineChunk.length <= limit) {
      currentLines = [line];
      continue;
    }
    for (const chunk of baseChunker(singleLineChunk, limit)) {
      if (chunk) {
        chunks.push(chunk);
      }
    }
