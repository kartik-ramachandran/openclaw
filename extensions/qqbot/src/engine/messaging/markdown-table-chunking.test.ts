...
describe("chunkQQBotMarkdownText", () => {
  ...
  it("keeps long fenced chunks under the QQ markdown safety limit", () => {
  ...
  it("splits fenced code chunks between lines for every viable limit", () => {
    ...
    const singleLineFenceLength = ["```ts", firstLine, "```"].join("\n").length;
    const wholeFenceLength = ["```ts", firstLine, secondLine, "```"].join("\n").length;
  ...
  it("handles prose before and after a table split at row boundaries", () => {
    ...
    expect(chunkQQBotMarkdownText(text, 70, baseChunker)).toEqual([
      "前置说明第一段，长度足够触发普通文本先发送。\n前置说明第二段继续解释。",
